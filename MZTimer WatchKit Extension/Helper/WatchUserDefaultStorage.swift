...
class WatchUserDefaultStorage {

    static let shared = WatchUserDefaultStorage()
...
//            iCalenderHelper.shared.addEvent(event: iCalenderEvent(title: "\(event.emoji) \(event.title)", note: "From MZTimer", time: event.time, startDate: event.startDate))
    ...
    public func saveContact(contact: Contact) {
        self.contact.append(contact)
        updateContact()
    }
    public func loadContact() -> [Contact]{
        return contact
    }
...
extension WatchUserDefaultStorage {
